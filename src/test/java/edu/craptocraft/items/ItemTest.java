...
public class ItemTest {
    ...
    public void testConstructorItem() {
        ...
        assertEquals("Coral Bits.....1.0$", bits.toString());

        Product rings = new Item("Kelp Rings", 1.50);
        assertEquals("Kelp Rings.....1.5$", rings.toString());

        Product loaf = new Item("Golden Loaf", 2.00);
        assertEquals("Golden Loaf.....2.0$", loaf.toString());

        Product soda = new Item("Seafoam Soda", 1.00);
        assertEquals("Seafoam Soda.....1.0$", soda.toString());
    }
